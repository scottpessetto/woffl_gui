...
def _run_frontier_optimization(
    well_configs, nozzles, throats, method, marginal_wc, n_pumps,
    *, n_steps=9, progress=None,
):
    ...
    env = plant.operating_envelope([best["total_pf"]], n_pumps, header_cap=_MAX_HEADER_PSI)[0]
    min_flow = plant.min_total_flow(n_pumps)
    meta = {
        "header_psi": best["P"], "total_pf_bpd": best["total_pf"],
        "total_oil_bopd": best["total_oil"], "frontier_cap_bpd": best["cap"],
        "n_pumps": n_pumps, "suction_psi": suction, "min_total_flow": min_flow,
        "recirc": best["total_pf"] < min_flow, "pumps": env["pumps"],
        "sweep": [{"header_psi": s["P"], "total_pf_bpd": s["total_pf"],
                   "total_oil_bopd": s["total_oil"]} for s in sweep],
        "nozzles": list(nozzles), "throats": list(throats),
    }
...
def _match_check_mpad(well_configs, current_choices, test_rates, n_pumps):
    ...
    pf = PowerFluidConstraint(total_rate=max(total_pf * 1.5, 80000.0),
...
def _evaluate_fixed_scenario(
    well_configs, choices, n_pumps, *, fallback_choices=None, test_rates=None,
    ...
    meta = {"header_psi": max(lo_p, min(_MAX_HEADER_PSI, ppf)), "total_pf_bpd": total_pf,
            "total_oil_bopd": total_oil, "over_capacity": over_cap, "n_pumps": n_pumps,
            "recirc": total_pf < plant.min_total_flow(n_pumps), "history": history}
    return per_well, meta


def _evaluate_existing_scenario(
    well_configs, scenario_choices, current_choices, n_pumps, *,
    ...
    scn_meta = {"header_psi": max(lo_p, min(_MAX_HEADER_PSI, header)), "total_pf_bpd": total_pf,
                "total_oil_bopd": total_oil, "over_capacity": over_cap, "n_pumps": n_pumps,
                "recirc": total_pf < plant.min_total_flow(n_pumps), "history": []}
...
def _render_configure() -> None:
    ...
    with c1:
        n_pumps = st.radio("HP pumps online", [3, 2, 1], horizontal=True,
                           key="mp_n_pumps",
    ...
    with c5:
        throats = st.multiselect("Throat ratios to test", THROAT_OPTIONS,
                                 default=["A", "B", "C", "D"], key="mp_throats")

    mn, mx = plant.min_total_flow(n_pumps), plant.max_total_flow(n_pumps)
    st.caption(f"With **{n_pumps} HP pump(s)**: min-flow (recirc) floor "
    ...
    op = ([{"label": "Last run", "total_pf_bpd": last_meta["total_pf_bpd"],
          ...
          if last_meta and last_meta.get("n_pumps") == n_pumps else None)
    if op:
    ...
    st.caption("Model each well at its current pump + chosen IPR vs recent tests. Fix ✗ first.")
    current = {
        w: (active[w].get("review_nozzle") or "", active[w].get("review_throat") or "")
        for w in active
    }
    sig = (n_pumps, tuple(sorted(
    ...
    if refresh or not mc or mc.get("sig") != sig:
        ...
        try:
            with st.spinner("Modeling each well at its current pump…"):
                rows, hdr = _match_check_mpad(wrs.store_to_well_configs(active), cur_ch, tr, n_pumps)
    ...
    if mc:
        ...
        st.caption(f"Modeled at header ≈ {mc['header']:,.0f} psi.  "
    ...
    if st.button("▶ Run optimization", type="primary", key="mp_run"):
        ...
        def _progress(it, max_it, P, total_pf, total_oil):
            bar.progress(min(it / max_it, 1.0),
...
def _render_results() -> None:
    ...
    for p in meta["pumps"]:
        if p.get("hz") is None:
            pump_rows.append({"Pump": p["name"], "Online": p.get("n", n_pumps),
                              "Speed": "—", "Amps/pump": "—", "Limit (A)": round(p["amp_limit"])})
            continue
        head = p["amp_limit"] - p["amps"]
        pump_rows.append({
            "Pump": p["name"], "Online": p.get("n", n_pumps), "Speed": f"{p['hz']:.1f} Hz",
            "Amps/pump": round(p["amps"], 1), "Limit (A)": round(p["amp_limit"]),
            "Headroom": f"{head:.0f} A ({head / p['amp_limit'] * 100:.0f}%)",
        })
    st.dataframe(pd.DataFrame(pump_rows), use_container_width=True, hide_index=True)

    if si_wells:
        st.info(f"**Optimizer shut in {len(si_wells)} well(s):** {', '.join(si_wells)}.")
    ...
    for w in si_wells:
        rows.append({"Well": w, "Nozzle": "—", "Throat": "—", "Oil (BOPD)": 0,
    ...
    if st.button("⬇ Download results CSV", key="mp_res_dl"):
        ...
        autodownload(df.to_csv(index=False).encode("utf-8"),
                     "M_pad_optimization_results.csv", "text/csv", "mp_res_auto_dl")

    st.markdown("#### HP-bank frontier")
    _render_frontier_plot(n_pumps, op_points=[{
...
def _render_scenario_comparator(results, meta, active, n_pumps) -> None:
    ...
    st.caption("Pick a pump (or shut-in) per well and see the resulting oil + header "
    ...
    base_choice = st.radio(
        ...
        horizontal=True, key="mp_cmp_base",
    ...
    preset = st.radio("Scenario", ["All current pumps", "All optimized (baseline)", "Custom per-well"],
                      horizontal=True, key="mp_scn_preset")
    if preset == "Custom per-well":
        ...
        if bc[1].button("⤓ Fill all from optimized", key="mp_scn_fill_opt"):
            ...
            st.session_state["mp_scn_nonce"] = nonce + 1
            st.rerun()
        bc[2].caption(f"Filled from **{'latest installed' if basis == 'current' else 'optimized'}** "
    ...
    if st.button("▶ Compute scenario & compare", type="primary", key="mp_scn_run"):
        ...
        try:
            with st.spinner("Running scenario (coupled to the frontier)…"):
                if is_existing:
                    cur_ch = {w: (current[w] if current[w][0] and current[w][1] else None)
                              for w in active}
                    tr = {w: _recent_test_rates(w) for w in active}
                    per_well, scn_meta = _evaluate_existing_scenario(
                        well_configs, choices, cur_ch, n_pumps, test_rates=tr, progress=_p)
                else:
                    per_well, scn_meta = _evaluate_fixed_scenario(
                        well_configs, choices, n_pumps, fallback_choices=dict(opt_choice), progress=_p)
        except Exception as e:
            bar.empty()
    ...
    if scn and scn_pw:
        ...
        cmp = pd.DataFrame({
            ...
            base_label: [base_meta["total_oil_bopd"], base_meta["total_pf_bpd"], base_meta["header_psi"]],
        ...
        if scn.get("recirc"):
            st.warning(f"Scenario total PF is below the {n_pumps}-pump min-flow floor — "
                       "recirc risk. Drop an HP pump or raise demand.")
        if scn.get("over_capacity"):
            st.warning("Scenario draws more PF than the HP bank can push at 3,500 — size some pumps down.")
        ...
        if subs:
            st.caption("⚠ Chosen pump infeasible — substituted: "
                       + ", ".join(f"{r['well']} ({r['pump']})" for r in subs))
        if starred:
            st.caption("★ Model couldn't solve — estimated from recent-test rate × ripple: " + ", ".join(starred))
        if infeas:
        ...
        bias_note = (" **Model÷Test** = model ÷ measured (>1 = loose IPR match)." if is_existing else "")
        st.caption(f"**Δ oil** = per-well change ({base_label.lower()} → scenario). "
        ...
        for w in sorted(active):
            ...
            row = {"Well": w, f"{base_label} pump": base_pump.get(w, "—"), f"{base_label} oil": b_oil}
...
def _render_frontier_plot(n_pumps, op_points=None) -> None:
    ...
    fig = go.Figure()
    if xs:
        fig.add_trace(go.Scatter(x=xs, y=ys, mode="lines", name=f"{n_pumps}-pump frontier (capped 3,500)",
                                 line=dict(color="#1f77b4"),
                                 hovertemplate="%{x:.0f} BPD<br>%{y:.0f} psi<extra></extra>"))
    fig.add_vrect(x0=(xs[0] if xs else 0), x1=mn, fillcolor="red", opacity=0.08, line_width=0,
                  annotation_text="recirc / min-flow", annotation_position="top left")
    fig.add_vline(x=mn, line=dict(color="#d62728", width=1.5, dash="dash"),
                  annotation_text=f"min-flow  {mn:,.0f} BPD", annotation_position="top right")
    for p in (op_points or []):
        fig.add_trace(go.Scatter(
            ...
            hovertemplate="%{x:.0f} BPD<br>%{y:.0f} psi<extra></extra>"))
    fig.update_layout(xaxis_title="Total power-fluid flow (BPD)",
...
def run_m_pad_page() -> None:
    ...
    st.caption("Review each Moose Pad well, then optimize the pad against the HP "
