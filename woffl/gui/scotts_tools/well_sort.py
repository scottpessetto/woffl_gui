...
from woffl.assembly.well_sort_engine import (
    DEFAULT_POPS_PADS as _DEFAULT_POPS_PADS,
    POPS_PUMP_HANDLES,
    PUMP_LIMIT_PRESETS,
    add_online_decision,
    add_shut_decision,
    effective_wc as _effective_wc,
    field_marginal_wc as _field_marginal_wc,
    form_basis_note as _form_basis_note,
    pad_marginal_wc as _pad_marginal_wc,
)
...
# _DEFAULT_POPS_PADS re-exported from well_sort_engine (single source).
...
# PUMP_LIMIT_PRESETS / POPS_PUMP_HANDLES moved to
# woffl.assembly.well_sort_engine (single source, shared with the web API);
...
def compute_field_marginal_wc(threshold_pct: float = 2.0) -> dict | None:
    ...
    Thin wrapper: fetches the online non-POPs table (cached) and delegates
    the canonical cumulative-water walk to
    ``woffl.assembly.well_sort_engine.field_marginal_wc`` (R-8 / P1-30: one
    implementation only). Public so the Batch Run "Import from Well Sort"
    button can call it without re-implementing the math.
    """
    return _field_marginal_wc(_build_online_non_pops(), threshold_pct)
...
def compute_pad_marginal_wc(pad: str, pump_limit: float) -> dict | None:
    ...
    Thin wrapper over ``well_sort_engine.pad_marginal_wc`` (the canonical
    implementation, shared with the web API) fed from the cached online
    table. See the engine docstring for the lift/total stream semantics.
    """
    return _pad_marginal_wc(_build_online_full(), pad, pump_limit)
...
# ---------------------------------------------------------------------------
#
# The decision engine (_effective_wc, add_online_decision, add_shut_decision)
# moved to woffl.assembly.well_sort_engine - pure DataFrame math shared with
# the web API - and is re-imported at the top of this module. Rendering below
# is unchanged.
