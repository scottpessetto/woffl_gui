...
def _run_coupled_optimization(
    well_configs, n_pumps, nozzles, throats, method, marginal_wc,
    *, max_iter=8, tol_psi=10.0, relax=0.6, progress=None,
):
    ...
    for it in range(max_iter):
        ...
        history.append({
            "iter": it + 1, "trial_psi": round(ppf_c, 1),
            "total_pf_bpd": round(total_pf, 0), "curve_psi": round(new_ppf, 1),
        })
        if progress:
...
def _recent_test_rates(well: str, n_recent: int = 5):
    ...
    try:
        ...
        oils = (pd.to_numeric(t["WtOilVol"], errors="coerce")
                if "WtOilVol" in t.columns else pd.Series(dtype=float))
        oils = oils[oils > 0].head(n_recent)
        pfs = (pd.to_numeric(t["lift_wat"], errors="coerce")
               if "lift_wat" in t.columns else pd.Series(dtype=float))
...
def _evaluate_fixed_scenario(
    ...
    for it in range(max_iter):
        ...
        for wc in well_configs:
            ...
            if perf is None and test_rates and wc.well_name in test_rates:
                ...
                per_well.append({
                    "well": wc.well_name, "pump": f"{ch[0]}{ch[1]} ★",
    ...
    meta = {
        "n_pumps": n_pumps, "header_psi": max(1000.0, min(5000.0, ppf)),
        "total_pf_bpd": total_pf, "total_oil_bopd": total_oil,
        "per_pump_bpd": s_pad_plant.per_pump_flow(total_pf, n_pumps),
        "in_range": s_pad_plant.flow_in_range(total_pf, n_pumps),
        "station_cap_bpd": cap, "history": history,
    }
...
def _evaluate_existing_scenario(
    ...
    header_base = (
        ...
        if total_pf_base > 0 else s_pad_plant.discharge_pressure(0.6 * cap, n_pumps)
    )
    ...
    for it in range(max_iter):
        ...
        for w in names:
            ...
            if ms and mcw and mcw[0] > 0 and mcw[1] > 0:
                ...
                per_well.append({"well": w, "pump": f"{ch[0]}{ch[1]}",
                                 "oil": float(ms["oil_rate"]), "pf": float(ms["lift_water"]),
                                 "note": ""})
...
def _match_check(well_configs, current_choices, n_pumps, test_rates):
    ...
    header = (s_pad_plant.discharge_pressure(total_pf, n_pumps)
              if total_pf > 0 else s_pad_plant.discharge_pressure(0.6 * cap, n_pumps))
...
def _render_configure() -> None:
    ...
    with c1:
        n_pumps = st.radio("Booster pumps online", [3, 2], horizontal=True,
                           key="sp_n_pumps",
    ...
    with c5:
        throats = st.multiselect("Throat ratios to test", THROAT_OPTIONS,
                                 default=["A", "B", "C", "D"], key="sp_throats")
    ...
    _render_curve_plot(n_pumps, op_points=[{
        "label": "Last run", "total_pf_bpd": op["total_pf_bpd"],
        "header_psi": op["header_psi"], "color": "#d62728",
    }] if op else None)
    ...
    if refresh or not mc or mc.get("sig") != sig:
        ...
        try:
            with st.spinner("Modeling each well at its current pump…"):
                rows, hdr = _match_check(wrs.store_to_well_configs(active), cur_ch, n_pumps, tr)
    ...
    if st.button("▶ Run optimization", type="primary", key="sp_run"):
        ...
        with st.spinner("Running batch sweeps + optimization (this can take a minute)…"):
        ...
        for k in ("sp_scenario_meta", "sp_scenario_per_well",
                  "sp_baseline_meta", "sp_baseline_per_well"):
            st.session_state.pop(k, None)
        st.session_state[_STAGE_KEY] = 2
        st.rerun()


def _parse_pump(label):
...
def _render_results() -> None:
    ...
    # Wells fed to the optimizer but NOT in the results were SHUT IN (MILP's
    # "at most one pump" let it drop them). Surface them instead of vanishing.
    ...
    if not meta["converged"]:
        st.warning(f"Pump-curve coupling {conv} in the iteration cap — review the trend below.")
    if si_wells:
        st.info(
            ...
            "the marginal water cut). Shown as SHUT IN below; test alternatives in "
            "the scenario comparator."
        )
    ...
    with c1:
        if st.button("⬇ Download results CSV", key="sp_res_dl"):
            ...
            autodownload(df.to_csv(index=False).encode("utf-8"),
                         "S_pad_optimization_results.csv", "text/csv", "sp_res_auto_dl")
    with c2:
        with st.expander("Pump-curve coupling iterations"):
            st.dataframe(pd.DataFrame(meta["history"]), use_container_width=True, hide_index=True)

    # Booster curve
    st.markdown("#### Booster curve")
    _render_curve_plot(meta["n_pumps"], op_points=[{
    ...
    base_choice = st.radio(
        ...
        horizontal=True, key="sp_cmp_base",
    ...
    preset = st.radio(
        ...
        horizontal=True, key="sp_scn_preset",
        ...
        with st.spinner("Running scenario (coupled to the booster curve)…"):
            if is_existing:
                ...
                per_well, scn_meta = _evaluate_existing_scenario(
                    well_configs, choices, cur_ch, meta["n_pumps"],
                    test_rates=tr, progress=_p,
                )
            else:
                per_well, scn_meta = _evaluate_fixed_scenario(
                    well_configs, choices, meta["n_pumps"],
                    fallback_choices=dict(opt_choice), progress=_p,
                )
        bar.empty()
    ...
    if is_existing:
        ...
        base_meta = {
            "n_pumps": n, "total_oil_bopd": sum(base_oil.values()),
            "total_pf_bpd": tot_pf,
            "header_psi": s_pad_plant.discharge_pressure(tot_pf, n) if tot_pf > 0 else 0.0,
    ...
    if scn and scn_pw:
        ...
        cmp = pd.DataFrame({
            "Metric": ["Total oil (BOPD)", "Total PF (BPD)",
                       "Header PF pressure (psi)", "Per-pump flow (BPD)"],
            base_label: [base_meta["total_oil_bopd"], base_meta["total_pf_bpd"],
                         base_meta["header_psi"], base_meta["per_pump_bpd"]],
            "Your scenario": [scn["total_oil_bopd"], scn["total_pf_bpd"],
                              scn["header_psi"], scn["per_pump_bpd"]],
        })
        ...
        _render_curve_plot(n, op_points=[
        ...
        st.caption(
            ...
            "test × the average ripple of the solving wells. Sorted by biggest mover." + bias_note
        )
...
def _render_curve_plot(n_pumps: int, op_points=None) -> None:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=xs, y=ys, mode="lines", name=f"{n_pumps}-pump curve",
    ...
    fig.add_vrect(x0=down_x, x1=up_x, fillcolor="green", opacity=0.08, line_width=0,
                  annotation_text="recommended window", annotation_position="top left")
    fig.add_vrect(x0=up_x, x1=x_max, fillcolor="red", opacity=0.07, line_width=0)
    fig.add_vline(x=down_x, line=dict(color="#888", width=1, dash="dot"),
                  annotation_text=f"down-thrust  {down_x:,.0f} BPD",
                  annotation_position="bottom right")
    fig.add_vline(x=up_x, line=dict(color="#d62728", width=1.5, dash="dash"),
    ...
    fig.update_layout(
        xaxis_title="Total station flow (BPD)", yaxis_title="Header pressure (psi)",
...
def run_s_pad_page() -> None:
    ...
    st.caption("Review each S-Pad well, then optimize the pad against the 3-pump booster curve.")
