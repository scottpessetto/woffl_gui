...
def milp_optimization(optimizer: "NetworkOptimizer") -> list["OptimizationResult"]:
    ...
    This is guaranteed to find the global optimum (unlike greedy, which
    can get stuck in a local optimum when trading power fluid between
    wells would yield more total oil).
    ...
    for well in optimizer.wells:
        ...
        for _, row in successful.iterrows():
            ...
            if perf is None:
                continue
            configs.append({"well_name": wn, "nozzle": row["nozzle"], "throat": row["throat"], "perf": perf})
    ...
    # A_well[i, k] = 1 if config k belongs to well i
...
def optimize(optimizer: "NetworkOptimizer", method: str = "milp") -> list["OptimizationResult"]:
    """Main optimization dispatcher

    Args:
        optimizer: NetworkOptimizer instance
        method: Optimization method ('milp', 'greedy', 'proportional')
    ...
    if method == "milp":
        ...
        raise ValueError(f"Unknown optimization method: {method}. Use 'milp', 'greedy', or 'proportional'")
