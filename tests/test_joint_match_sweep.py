...
from woffl.gui.joint_match import _inflow_from_qmax, joint_match
...
def main():
    ...
    for nozzle, throat, wc, qmax, ppf in combos:
        ...
        if dt > 6:
            slow.append(
                (f"{nozzle}{throat} wc{wc} q{qmax:.0f} pf{ppf:.0f}", dt, r.iterations)
            )
        if r.status == "matched":
            n_match += 1
        elif r.status == "partial":
            n_partial += 1
            fails.append((f"{nozzle}{throat} wc{wc:.2f} q{qmax:.0f} pf{ppf:.0f}", t, r))
        else:
            n_fail += 1
            fails.append((f"{nozzle}{throat} wc{wc:.2f} q{qmax:.0f} pf{ppf:.0f}", t, r))
    ...
    if fails:
        ...
        for name, t, r in fails[:25]:
            print(
                f"    {name}: truth oil={t['oil']:.0f} pf={t['pf']:.0f} "
...
if __name__ == "__main__":
    # Scoped to the script run only (guarded under __main__ so pytest
    # collection/import never touches process env) and restored afterward so
    # a shared interpreter session can't leak WOFFL_MAX_WORKERS into anything
    # else that imports this module later.
    _prior_workers = os.environ.get("WOFFL_MAX_WORKERS")
    os.environ.setdefault("WOFFL_MAX_WORKERS", "1")
    try:
        main()
    finally:
        if _prior_workers is None:
            os.environ.pop("WOFFL_MAX_WORKERS", None)
        else:
            os.environ["WOFFL_MAX_WORKERS"] = _prior_workers
