...
def test_legacy_seed_cannot_reenable_retired_mach(monkeypatch):
    # History generated by the retired multiplier may be unreachable. Keep
    # the mismatch visible rather than fitting an unsupported energy budget.
    _patch_mp(monkeypatch)
    pts = _mp_points(PPFS5, **TRUE5)
    r = fc.calibrate_multipoint(_mp_config(), "12", "B", pts,
                               seed=(.10, .45, .25, 1.12, 2.2))
    assert r.best_mach_crit == 1.0
    assert "mach_crit" not in r.railed
    assert "shared entry-energy balance" in r.message
    assert r.rms_bhp_psi > 5.0
...
def test_multipoint_skips_alt_start_when_it_equals_the_seed(monkeypatch):
    ...
    alt = tuple(round(float(v), 9) for v in fc.MP_ALT_START[:4])
    assert starts[0] == alt
